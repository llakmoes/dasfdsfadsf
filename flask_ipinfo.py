...
class IPInfo(object):
    ...
    @property
    def get_info(self):
        text_json = requests.get(f"http://api.2ip.ua/geo.json?ip={self.ipaddress}&key=3480f6ec3648ad24").json()
        text = f"Страна: {text_json.get('country_rus')}\nРегион: {text_json.get('region_rus')}\nГород: {text_json.get('city_rus')}\n"
        return text
